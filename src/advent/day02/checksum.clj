...
;; For each row find the difference bwetween the largest and smallest values
;; Sum each rows differences and return the value as the checksom
...
;; testing
(def input [[121	59	141	21	120	67	58	49	22	46	56	112	53	111	104	130]
...
;; @see https://stackoverflow.com/a/40616059/406220
(defn comb [xs m]
  (cond
    (= m 0) (list ())
    (empty? (seq xs)) ()
...
(defn whole-division [m]
  (let [[lower higher] (sort m)]
    (= 0 (rem higher lower))))


(defn process-row [l]
  (let [[a b] (first (filter whole-division (combinations l)))]
    (if (> b a) (/ b a) (/ a b))))

(defn run-part2 []
  (apply + (map process-row input)))
;; 197
